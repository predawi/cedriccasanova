...
class TT_ImportData{
    ...
    public function demo_import_page(){
        ...
        $templates = '';
        $file_dir = TT::file_require( get_stylesheet_directory().'/includes/demo-data/thumbs/' );
        foreach( glob( $file_dir . '*.png' ) as $filename ) {
            $filename = basename($filename);
            $fnames = $this->parseFilename($filename);
            $img = TT::file_require( trailingslashit(get_template_directory_uri()).'includes/demo-data/thumbs/'.$filename, true );
            $templates .= '<label>
        ...
        echo '<div class="wrap"><div id="icon-tools" class="icon32"></div>';
            ...
            echo '<form method="post">
                ...
                <tr>
                    <th scope="row"><label>Import Widgets:</label></th>
                    <td>
                        <label>
                            <input type="checkbox" name="import_widgets" value="1" /> Import Demo Site Widgets
                        </label>
                    </td>
                </tr>
    ...
    public function import_data($param){
        ...
        require_once( get_template_directory() . "/framework/classes/lib/wxr-importer/class-logger.php" );
        require_once( get_template_directory() . "/framework/classes/lib/wxr-importer/class-logger-cli.php" );
        require_once( get_template_directory() . "/framework/classes/lib/wxr-importer/class-wxr-importer.php" );
        ...
        $main_content_file = TT::file_require(get_template_directory().'/includes/demo-data/main-content.xml');
        if( file_exists($main_content_file) ){
            $main_content_file = TT::file_require(get_template_directory_uri().'/includes/demo-data/main-content.xml');
            $main_content_obj = wp_remote_get($main_content_file);
            $main_contents = array_key_exists('body', $main_content_obj) ? $main_content_obj['body'] : '';
        ...
        if( isset($param['template']) && !empty($param['template']) ){
            ...
            // Set Primary Menu
            $navigation_file = TT::file_require(get_template_directory().'/includes/demo-data/navigation.txt');
            if( file_exists($navigation_file) ){
                $navigation_file = TT::file_require(get_template_directory_uri().'/includes/demo-data/navigation.txt');
                $navigation_obj = wp_remote_get($navigation_file);
                $nav_contents = array_key_exists('body', $navigation_obj) ? $navigation_obj['body'] : '';
